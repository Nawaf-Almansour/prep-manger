...
import { useCreateInventoryItem, InventoryFormDataWithImage } from '@/hooks/useInventory';
...
export default function AddInventoryPage() {
  ...
  const { data: categories = [] } = useCategories(true); // Get active categories only
  ...
  const onSubmit = (data: InventoryFormData) => {
    ...
    const submitData: InventoryFormDataWithImage = {
      ...data,
    ...
    <DashboardLayout allowedRoles={['supervisor', 'manager']}>
      <div className="max-w-3xl mx-auto space-y-6">
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-3xl font-bold">{t('inventory.addItem')}</h1>
            <p className="text-gray-500 mt-1">{t('inventory.addIngredient')}</p>
          </div>
        </div>
